...
class Counters extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<AppCubic , AppStates>(builder: (BuildContext context , state){
      AppCubic cubicParam = AppCubic.get(context);
      return Scaffold(
                  body:Center(child: Row(
                    ...
                    children: [
                      ...
                      const Spacer(),
                      Text(cubicParam.number.toString() , style: const TextStyle(fontSize: 50),),
                      const Spacer(),
                      ElevatedButton.icon(
                        onPressed: () {
                          cubicParam.decrease();
